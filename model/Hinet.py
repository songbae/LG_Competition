...
def conv3x3(in_chn, out_chn, bias=True):
    layer = nn.Conv2d(in_chn, out_chn, kernel_size=3,
...
class HINet(nn.Module):
    ...
    def __init__(self, in_chn=3, wf=64, depth=5, relu_slope=0.2, hin_position_left=3, hin_position_right=4):
        ...
        self.conv_01 = nn.Conv2d(6, wf, 3, 1, 1)
        self.conv_02 = nn.Conv2d(6, wf, 3, 1, 1)
        ...
        for i in range(depth):  # 0,1,2,3,4
            ...
            self.down_path_2.append(UNetConvBlock(
                prev_channels, (2**i) * wf, downsample, relu_slope, use_csff=downsample, use_HIN=use_HIN))
        ...
        for i in reversed(range(depth - 1)):
            self.up_path_1.append(UNetUpBlock(
                prev_channels, (2**i)*wf, relu_slope))
    ...
    def forward(self, x,add_img):
        add_image=add_img
        image = x
        start_img=torch.cat([image,add_image],dim=1)
        #stage 1
        x1 = self.conv_01(start_img)
        encs = []
        decs = []
